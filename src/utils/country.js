...
export const countryOptions = () => {
  const options = [];
  for (const e of countryData) {
    options.push({ label: e.name, value: e.code2 });
  }
  return options;
};
...
export const stateCode = (country, stateName) => {
  for (const e of countryData) {
    if (country === e.code2) {
      for (const ee of e.states) {
        if (ee.name === stateName) return ee.code;
      }
    }
  }
  return '';
