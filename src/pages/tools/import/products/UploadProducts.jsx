...
const UploadProducts = () => {
  const props = {
    name: 'products',
    action: `${REACT_APP_API_SERVER}/api/admin/import/products`,
    accept: '.csv',
    headers: {
    ...
    onChange(info) {
      ...
      if (info.file.status === 'done') {
        message.success('Imported the products successfully');
      ...
      <Row>
        <Col span={24}>
          <h4 className={`${styles.title}`}>
            {t('pages.import.uploadProducts', 'Upload Products')}
          </h4>
        </Col>
      </Row>
