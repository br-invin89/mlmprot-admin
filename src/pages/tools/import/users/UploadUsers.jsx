...
const UploadUsers = () => {
  const props = {
    name: 'users',
    action: `${REACT_APP_API_SERVER}/api/admin/import/users`,
    accept: '.csv',
    headers: {
    ...
    onChange(info) {
      ...
      if (info.file.status === 'done') {
        message.success('Imported the users successfully');
      ...
      <Row>
        <Col span={24}>
          <h4 className={`${styles.title}`}>
            {t('pages.import.uploadUsers', 'Upload Users')}
          </h4>
        </Col>
      </Row>
